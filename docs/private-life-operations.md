...
The domain engines, safety boundaries, accessible controls, incident runbooks,
and synthetic calibration baseline are implemented. Live clinical and
financial providers still need separate certification. Physical camera and OCR
quality require supported-hardware evidence. A genuinely opted-in human pilot,
representative accessibility feedback, and explicit human support decisions are
required before these packages can be described as supported.

Review the [current status](current-status.md),
[privacy and security](privacy-security.md), or
